...
/**
 * One row from ANALYTICS.CONCIERGE.LEADS_WITH_POSITIVE_STATUS scoped to a
 * single (team_id, event_id) — i.e. one campaign. The status here is
 * the platform's lead_interest_status_name (Meeting Booked / Meeting Initiated /
 ...
 * time per the side-store decision in docs/ARCHITECTURE.md.
 */
export interface CampaignLead {
  ...
  company: string | null;
  phone: string | null;
  /** Onsite Contact Manager — the platform rep who booked the meeting with this lead. */
  ocm: string | null;
  /** Customer Success Manager — the platform rep who owns the client relationship. */
  csm: string | null;
...
/**
 * One client is absent from the leads view, so its meetings come from a
 * dedicated "sidecar" table instead. Same columns, different names — aliased
 * below so the rest of the pipeline is unchanged. Both values come from env so
 * the special case is off unless configured.
 * ponytail: one-client special case; fold back into the main query if that
 * client ever lands in the leads view.
 */
const SIDECAR_TEAM_ID = process.env.SIDECAR_LEADS_TEAM_ID || null;
const SIDECAR_TABLE =
  process.env.SIDECAR_LEADS_TABLE || "ANALYTICS.CONCIERGE.SIDECAR_LEADS";

/** Sidecar column names → the names the shared LeadRow mapper expects. */
const SIDECAR_LEAD_COLUMNS = `
  LEAD_ID,
  CAMPAIGN_ID,
  LEAD_NAME,
  TITLE,
...
export async function listLeadsForCampaign(
  ...
  if (teamId === SIDECAR_TEAM_ID) {
    ...
    const rows = await query<LeadRow>(
      ...
      logger.error("sidecar.leads_unavailable", {
        eventId,
  ...
  const rows = await query<LeadRow>(
    `SELECT ${LEAD_COLUMNS}
     FROM ANALYTICS.CONCIERGE.LEADS_WITH_POSITIVE_STATUS
     WHERE TEAM_ID = ?
       AND EVENT_ID = ?
 ...
 * is ANALYTICS.CONCIERGE.POSITIVE_LEAD_DETAILS (full population, not just leads we've
...
export async function hasCrossTeamConflict(
  ...
  try {
    const rows = await query<{ N: number }>(
      `SELECT COUNT(*) AS "N"
       FROM ANALYTICS.CONCIERGE.POSITIVE_LEAD_DETAILS
...
export async function listLeadsForEventAllTeams(
  ...
  const viewTeams = teams.filter((t) => t.teamId !== SIDECAR_TEAM_ID);
  const hasSidecar = teams.length !== viewTeams.length;
  ...
  const [viewRows, sidecarRows] = await Promise.all([
    viewTeams.length > 0
      ? query<LeadRow & { TEAM_ID: string }>(
          `SELECT ${LEAD_COLUMNS},
           ...
           FROM ANALYTICS.CONCIERGE.LEADS_WITH_POSITIVE_STATUS
           WHERE EVENT_ID = ?
             AND TEAM_ID IN (${viewTeams.map(() => "?").join(", ")})
             AND STATUS = 'Meeting Booked'`,
          [eventId, ...viewTeams.map((t) => t.teamId)]
        )
      : Promise.resolve([]),
    hasSidecar
      ? query<LeadRow & { TEAM_ID: string }>(
          `SELECT ${SIDECAR_LEAD_COLUMNS},
           ...
           WHERE EVENT_ID = ?
             AND TEAM_ID = ?
             AND LEAD_INTEREST_STATUS_NAME = 'Meeting Booked'`,
          [eventId, SIDECAR_TEAM_ID]
        ).catch(() => [])
      : Promise.resolve([])
  ]);

  const leads = [...viewRows, ...sidecarRows].map(toEventLead);
...
export async function getLeadById(
  ...
  // Sidecar-client leads don't exist in the leads view — fall back to their table so
  // mark-no-show and the Slack OCM/CSM lookup still work for them.
  const sidecar = await query<LeadRow>(
    `SELECT ${SIDECAR_LEAD_COLUMNS}
     FROM ${SIDECAR_TABLE}
     WHERE LEAD_ID = ?
     LIMIT 1`,
  ...
  return sidecar.length > 0 ? toDomain(sidecar[0]) : null;
}

/**
 * The leads view returns meeting date and time as separate fields plus a
...
/**
 * Maps the platform's lead_interest_status_name to a concierge-friendly summary.
 ...
 */
export function sourceStatusToBadge(status: string | null): string {
  if (!status) return "—";
  return status;
}
  ...
  try {
    const rows = await query<{ W: string | null }>(
      `SELECT INSTANTLY_WORKSPACE_ID AS "W"
       FROM ANALYTICS.CONCIERGE.V_TEAM_DETAILS
       WHERE TEAM_ID = ?
       LIMIT 1`,
      [teamId]
    );
...
const SCHEDULER_HOST =
  process.env.SCHEDULER_BASE_URL || "https://scheduler.example.com/rsvp/hosted/";

/**
 * The native platform scheduler link for a lead's campaign, with concierge
 * UTM correlation (utm_content = sourceLeadId flows through to the booking
 * webhook). Returns null when the campaign has no active scheduler — callers
 * treat that as "no link to offer". Only ever emits native scheduler links.
 *
  ...
  try {
    const rows = await query<{ SLUG: string }>(
      `SELECT et.SLUG AS "SLUG"
       FROM APPDB.SCHEDULER.MEETING_HOST_EVENT_TYPES et
       JOIN APPDB.SCHEDULER.EVENTS_MEETINGSUBCAMPAIGN sc
         ON et.SUBCAMPAIGN_ID = sc.ID AND sc._FIVETRAN_DELETED = FALSE
       WHERE et._FIVETRAN_DELETED = FALSE
         AND et.STATUS = 'active'
         AND sc.UUID IN (
           SELECT SUB_CAMPAIGN_ID
           FROM ANALYTICS.CONCIERGE.V_SUB_CAMPAIGN_CONFIG
    ...
    return `${SCHEDULER_HOST}${rows[0].SLUG}?utm_source=concierge&utm_content=${encodeURIComponent(sourceLeadId)}`;
  } catch {
 ...
 * BOOKING_LINK field on the platform campaign config.
 *
    ...
    const rows = await query<{ TEAM_ID: string; BOOKING_LINK: string | null }>(
      `SELECT TEAM_ID AS "TEAM_ID", BOOKING_LINK AS "BOOKING_LINK"
       ...
       WHERE EVENT_ID = ?
         ...
         AND (SUB_CAMPAIGN_NAME ILIKE '%early confirmed%'
              OR SUB_CAMPAIGN_NAME ILIKE '%predicted%')
       ORDER BY
         IFF(SUB_CAMPAIGN_NAME ILIKE '%early confirmed%', 0, 1),
 ...
 *   { agent_emails: [...], agent_first_name: "Jordan", agent_last_name: "Royale", ... }
